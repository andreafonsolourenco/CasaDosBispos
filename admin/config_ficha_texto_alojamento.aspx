<%@ Page Language="C#" AutoEventWireup="true" CodeFile="config_ficha_texto_alojamento.aspx.cs" Inherits="config_ficha_textos" %>
...
<head>
    ...
    <meta name="description" content="Casa dos Bispos Software - Ficha do Texto da Secção Alojamento">
    <meta name="author" content="André Lourenço">
    <title>Casa dos Bispos Software - Ficha do Texto da Secção Alojamento</title>
    <!-- Favicon -->
...
<body>
    ...
    <div class="main-content">
        ...
        <nav class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
            <div class="container-fluid">
                <!-- Brand -->
                <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block">Alojamento - Texto</a>
                <!-- User -->
            ...
            <div class="container-fluid d-flex align-items-center">
                <div class="row">
                    <div class="col-lg-12 col-md-10">
                        <h1 class="display-2 text-white">Alojamento - Texto</h1>
                        <p class="text-white mt-0 mb-5">Crie/Edite o texto presente no Site para o separador "Alojamento"</p>
                        <a href="#!" class="btn btn-secondary" onclick="confirmSave();">Guardar alterações</a>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="card-header bg-white border-0">
                            <div class="row align-items-center">
                                <table style="width: 100%; margin-left: 15px">
                                    <tr>
                                        <td style="width: 100%">
                                            <h3 class="mb-0" runat="server" id="tableTitle">Alojamento - Texto</h3>
                                        </td>
                                    </tr>
                                </table>
                            </div>
    ...
    <script>
        ...
        function getData() {
            $.ajax({
                type: "POST",
                url: "config_ficha_texto_alojamento.aspx/getData",
                //data: '{"id":"' + id + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (res) {
                    if(res.d == '') {
                        $('#txtCodigo').val('ALOJAMENTO');
                        $('#txtOrdem').val('99');
                        $('#txtNome').val('Alojamento');
                        $('#txtNomeEn').val('');
                        $('#txtNomeFr').val('');
                        $('#txtNomeEs').val('');
                        $('#txtTexto').val('');
        ...
        function saveData() {
            ...
            nome = nome.replaceAll('\'', '\'\'');
            nome = nome.replaceAll('\'\'\'\'', '\'\'');
            nomeEn = nomeEn.replaceAll('\n', '<br />');
            nomeEn = nomeEn.replaceAll('\'', '\'\'');
            nomeEn = nomeEn.replaceAll('\'\'\'\'', '\'\'');
            nomeFr = nomeFr.replaceAll('\n', '<br />');
            nomeFr = nomeFr.replaceAll('\'', '\'\'');
            nomeFr = nomeFr.replaceAll('\'\'\'\'', '\'\'');
            nomeEs = nomeEs.replaceAll('\n', '<br />');
            nomeEs = nomeEs.replaceAll('\'', '\'\'');
            nomeEs = nomeEs.replaceAll('\'\'\'\'', '\'\'');
            ...
            texto = texto.replaceAll('\'', '\'\'');
            texto = texto.replaceAll('\'\'\'\'', '\'\'');
            textoEn = textoEn.replaceAll('\n', '<br />');
            textoEn = textoEn.replaceAll('\'', '\'\'');
            textoEn = textoEn.replaceAll('\'\'\'\'', '\'\'');
            textoFr = textoFr.replaceAll('\n', '<br />');
            textoFr = textoFr.replaceAll('\'', '\'\'');
            textoFr = textoFr.replaceAll('\'\'\'\'', '\'\'');
            textoEs = textoEs.replaceAll('\n', '<br />');
            textoEs = textoEs.replaceAll('\'', '\'\'');
            textoEs = textoEs.replaceAll('\'\'\'\'', '\'\'');

            $.ajax({
                type: "POST",
                url: "config_ficha_texto_alojamento.aspx/saveData",
                data: '{"codigo":"' + codigo + '","nome":"' + nome + '","nome_en":"' + nomeEn + '","nome_fr":"' + nomeFr + '","nome_es":"' + nomeEs
                    + '","texto":"' + texto + '","texto_en":"' + textoEn + '","texto_fr":"' + textoFr + '","texto_es":"' + textoEs + '","ordem":"' + ordem
                ...
                success: function (res) {
                    ...
                    if (parseInt(dados[0]) > 0) {
                        loadUrl('config_ficha_texto_alojamento.aspx');
                    }
                    else {
