<%@ Page Language="C#" AutoEventWireup="true" CodeFile="lista_emails_recebidos.aspx.cs" Inherits="lista_emails_recebidos" %>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="Casa dos Bispos Software - Emails Recebidos">
    <meta name="author" content="André Lourenço">
    <title>Casa dos Bispos Software - Emails Recebidos</title>
    <!-- Favicon -->
...
<body>
    ...
    <div class="main-content">
        ...
        <nav class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
            <div class="container-fluid">
                <!-- Brand -->
                <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block">Emails Recebidos</a>
                <!-- Form -->
                                ...
                                <tr>
                                    <td style="width: 65%; max-width: 50% !important;">
                                        <div style="float:left">
                                            <h3 class="mb-0">Emails Recebidos</h3>
                                        </div>
                                    </td>
    ...
    <script>
        ...
        function getData() {
            loadingOn('A carregar os emails recebidos. Por favor aguarde...');
            var pesquisa = $('#txtPesquisa').val();
            var order = "";

            if (ordData == 0 && ordNome == 0 && ordEmail == 0 && ordAssunto == 0 && ordSucesso == 0) {
                order = ' ORDER BY ctrldata desc ';
            }
            else {
                order = ' ORDER BY ';

                if (ordData != 0) {
                    order += ordData == -1 ? ' ctrldata desc ' : ' ctrldata asc ';
                }
                ...
                if (ordAssunto != 0) {
                    order += ordAssunto == -1 ? ' assunto desc ' : ' assunto asc ';
                }

                if (ordSucesso != 0) {
                    order += ordSucesso == -1 ? ' sucesso desc ' : ' sucesso asc ';
                }
            }

            $.ajax({
                type: "POST",
                url: "lista_emails_recebidos.aspx/getGrelha",
        ...
        function visualizar(id) {
            $.ajax({
                type: "POST",
                url: "lista_emails_recebidos.aspx/showEmail",
                data: '{"id":"' + id + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (res) {
        ...
        function ordenaAssunto() {
            ...
            if (ordAssunto == 0) {
                ...
                ordAssunto = ordAssunto * (-1);
            }

            getData();
        }

        function ordenaSucesso() {
            ordData = 0;
            ordNome = 0;
            ordEmail = 0;
            ordAssunto = 0;
